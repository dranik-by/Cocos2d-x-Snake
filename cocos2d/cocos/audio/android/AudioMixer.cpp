...
: mTrackNames(0)
, mConfiguredNames((maxNumTracks >= 32 ? 0 : 1 << maxNumTracks) - 1)
, mSampleRate(sampleRate)
{
    ALOGVV("AudioMixer constructed, frameCount: %d, sampleRate: %d", (int)frameCount, (int)sampleRate);
    ALOG_ASSERT(maxNumTracks <= MAX_NUM_TRACKS, "maxNumTracks %u > MAX_NUM_TRACKS %u", maxNumTracks, MAX_NUM_TRACKS);
...
int AudioMixer::getTrackName(audio_channel_mask_t channelMask, audio_format_t format, int sessionId)
{
    ...
    if (names != 0)
    {
        ...
        t->mMixerChannelMask = audio_channel_mask_from_representation_and_bits(AUDIO_CHANNEL_REPRESENTATION_POSITION,
...
bool AudioMixer::setChannelMasks(int name, audio_channel_mask_t trackChannelMask, audio_channel_mask_t mixerChannelMask)
{
    ...
    ALOGE_IF(status != OK, "prepareForDownmix error %d, track channel mask %#x, mixer channel mask %#x", status,
             track.channelMask, track.mMixerChannelMask);
 ...
 */
static inline bool setVolumeRampVariables(float newVolume, int32_t ramp, int16_t* pIntSetVolume,
                                          int32_t* pIntPrevVolume, int32_t* pIntVolumeInc, float* pSetVolume,
                                          float* pPrevVolume, float* pVolumeInc)
{
    ...
    if (ramp != 0)
    {
        ...
        && maxv + inc != maxv)
        { // inc must make forward progress
            *pVolumeInc = inc;
            // ramp is set now.
            ...
            switch (param)
            {
                case AUXLEVEL:
                    if (setVolumeRampVariables(*reinterpret_cast<float*>(value),
                                               target == RAMP_VOLUME ? mState.frameCount : 0, &track.auxLevel,
                                               &track.prevAuxLevel, &track.auxInc, &track.mAuxLevel,
                                               &track.mPrevAuxLevel, &track.mAuxInc))
                    {
                        ALOGV("setParameter(%s, AUXLEVEL: %04x)", target == VOLUME ? "VOLUME" : "RAMP_VOLUME",
                              track.auxLevel);
                        invalidateState(1 << name);
                    }
                    break;
                default:
                    if ((unsigned)param >= VOLUME0 && (unsigned)param < VOLUME0 + MAX_NUM_VOLUMES)
                    {
                        if (setVolumeRampVariables(*reinterpret_cast<float*>(value),
                                                   ...
                                                   &track.volumeInc[param - VOLUME0], &track.mVolume[param - VOLUME0],
                                                   &track.mPrevVolume[param - VOLUME0],
                                                   &track.mVolumeInc[param - VOLUME0]))
                        {
                            ALOGV("setParameter(%s, VOLUME%d: %04x)", target == VOLUME ? "VOLUME" : "RAMP_VOLUME",
                                  param - VOLUME0, track.volume[param - VOLUME0]);
                            invalidateState(1 << name);
                        }
                    }
                    else
        ...
        case TIMESTRETCH:
            switch (param)
            {
                case PLAYBACK_RATE:
                {
                    ...
                    ALOGW_IF(!isAudioPlaybackRateValid(*playbackRate), "bad parameters speed %f, pitch %f",
                             playbackRate->mSpeed, playbackRate->mPitch);
                    if (track.setPlaybackRate(*playbackRate))
                    {
                        ALOGV("setParameter(TIMESTRETCH, PLAYBACK_RATE, STRETCH_MODE, FALLBACK_MODE "
                              "%f %f %d %d", playbackRate->mSpeed, playbackRate->mPitch, playbackRate->mStretchMode,
        ...
        if (sampleRate != trackSampleRate)
        {
            ...
            if (resampler == NULL)
            {
                ...
                ALOGVV("Creating resampler:"
                       ...
                       resamplerChannelCount, devSampleRate, quality);
...
inline void AudioMixer::track_t::adjustVolumeRamp(bool aux, bool useFloat)
{
    if (useFloat)
    {
        ...
        {
            if ((mVolumeInc[i] > 0 && mPrevVolume[i] + mVolumeInc[i] >= mVolume[i]) || (mVolumeInc[i] < 0 && mPrevVolume[i] + mVolumeInc[i] <= mVolume[i]))
            {
                volumeInc[i] = 0;
    ...
    else
    {
        for (uint32_t i = 0; i < MAX_NUM_VOLUMES; i++)
        {
            if (((volumeInc[i] > 0) && (((prevVolume[i] + volumeInc[i]) >> 16) >= volume[i])) || ((volumeInc[i] < 0) && (((prevVolume[i] + volumeInc[i]) >> 16) <= volume[i])))
            {
                volumeInc[i] = 0;
                prevVolume[i] = volume[i] << 16;
                mVolumeInc[i] = 0.;
    ...
    if (aux)
    {
        if (((auxInc > 0) && (((prevAuxLevel + auxInc) >> 16) >= auxLevel)) || ((auxInc < 0) && (((prevAuxLevel + auxInc) >> 16) <= auxLevel)))
        {
            auxInc = 0;
...
{
    ALOGW_IF(!state->needsChanged, "in process__validate() but nothing's invalid");
    ...
    while (en)
    {
        ...
        else
        {
            ...
            if (n & NEEDS_RESAMPLE)
            {
                all16BitsStereoNoResample = false;
                resampling = true;
                t.hook = getTrackHook(TRACKTYPE_RESAMPLE, t.mMixerChannelCount, t.mMixerInFormat, t.mMixerFormat);
            ...
            else
            {
                ...
                {
                    t.hook = getTrackHook((t.mMixerChannelMask == AUDIO_CHANNEL_OUT_STEREO  // TODO: MONO_HACK
                ...
                {
                    t.hook = getTrackHook(TRACKTYPE_NORESAMPLE, t.mMixerChannelCount, t.mMixerInFormat, t.mMixerFormat);
            ...
            if (all16BitsStereoNoResample && !volumeRamp)
            {
                if (countActiveTracks == 1)
                {
                    ...
                    if ((t.needs & NEEDS_MUTE) == 0)
                    {
                        ...
                        state->hook = getProcessHook(PROCESSTYPE_NORESAMPLEONETRACK, t.mMixerChannelCount,
    ...
    ALOGV("mixer configuration change: %d activeTracks (%08x) "
          "all16BitsStereoNoResample=%d, resampling=%d, volumeRamp=%d", countActiveTracks, state->enabledTracks,
    ...
    if (countActiveTracks > 0)
    {
        ...
        else if (all16BitsStereoNoResample)
        {
            if (countActiveTracks == 1)
            {
                ...
                state->hook = getProcessHook(PROCESSTYPE_NORESAMPLEONETRACK, t.mMixerChannelCount, t.mMixerInFormat,
                                             t.mMixerFormat);
            }
        }
    }
...
void AudioMixer::track__nop(track_t* t __unused, int32_t* out __unused, size_t outFrameCount __unused, int32_t* temp
                            __unused, int32_t* aux __unused)
{
}
...
void AudioMixer::track__16BitsMono(track_t* t, int32_t* out, size_t frameCount, int32_t* temp __unused, int32_t* aux)
{
...
void AudioMixer::process__nop(state_t* state, int64_t pts)
{
    ...
    while (e0)
    {
        ...
        int i = 31 - __builtin_clz(e1);
        {
            ...
            memset(t1.mainBuffer, 0,
                   state->frameCount * t1.mMixerChannelCount * audio_bytes_per_sample(t1.mMixerFormat));
        }
        ...
        while (e1)
        {
            ...
            e1 &= ~(1 << i);
            {
                ...
                while (outFrames)
                {
                    ...
                    int64_t outputPTS = calculateOutputPTS(t3, pts, state->frameCount - outFrames);
                    t3.bufferProvider->getNextBuffer(&t3.buffer, outputPTS);
                    if (t3.buffer.raw == NULL)
                        break;
        ...
        do
        {
            ...
            while (e2)
            {
                ...
                while (outFrames)
                {
                    ...
                    if (inFrames > 0)
                    {
                        t.hook(&t, outTemp + (BLOCKSIZE - outFrames) * t.mMixerChannelCount, inFrames,
                    ...
                    if (t.frameCount == 0 && outFrames)
                    {
                        ...
                        int64_t outputPTS = calculateOutputPTS(t, pts, numFrames + (BLOCKSIZE - outFrames));
            ...
            out = reinterpret_cast<int32_t*>((uint8_t*)out + BLOCKSIZE * t1.mMixerChannelCount * audio_bytes_per_sample(
            t1.mMixerFormat));
            numFrames += BLOCKSIZE;
...
void AudioMixer::process__genericResampling(state_t* state, int64_t pts)
{
    ...
    while (e0)
    {
        ...
        while (e1)
        {
            ...
            else
            {
                ...
                while (outFrames < numFrames)
                {
                    ...
                    t.hook(&t, outTemp + outFrames * t.mMixerChannelCount, t.buffer.frameCount, state->resampleTemp,
                           aux);
        ...
        convertMixerFormat(out, t1.mMixerFormat, outTemp, t1.mMixerInFormat, numFrames * t1.mMixerChannelCount);
    }
}
...
void AudioMixer::process__OneTrack16BitsStereoNoResampling(state_t* state, int64_t pts)
{
    ...
    while (numFrames)
    {
        ...
        if (in == NULL || (((uintptr_t)in) & 3))
        {
            ...
            ALOGE_IF((((uintptr_t)in) & 3), "process__OneTrack16BitsStereoNoResampling: misaligned buffer"
                                            " %p track %d, channels %d, needs %08x, volume %08x vfl %f vfr %f", in, i,
                     t.channelCount, t.needs, vrl, t.mVolume[0], t.mVolume[1]);
            return;
        }
...
int64_t AudioMixer::calculateOutputPTS(const track_t &t, int64_t basePTS, int outputFrameIndex)
{
...
static void volumeRampMulti(uint32_t channels, TO* out, size_t frameCount, const TI* in, TA* aux, TV* vol,
                            const TV* volinc, TAV* vola, TAV volainc)
{
    switch (channels)
    {
        case 1:
        ...
        case 2:
            ...
            break;
        case 3:
            volumeRampMulti<MIXTYPE_MONOVOL(MIXTYPE), 3>(out, frameCount, in, aux, vol, volinc, vola, volainc);
            break;
        case 4:
            volumeRampMulti<MIXTYPE_MONOVOL(MIXTYPE), 4>(out, frameCount, in, aux, vol, volinc, vola, volainc);
            break;
        case 5:
            volumeRampMulti<MIXTYPE_MONOVOL(MIXTYPE), 5>(out, frameCount, in, aux, vol, volinc, vola, volainc);
            break;
        case 6:
            volumeRampMulti<MIXTYPE_MONOVOL(MIXTYPE), 6>(out, frameCount, in, aux, vol, volinc, vola, volainc);
            break;
        case 7:
            volumeRampMulti<MIXTYPE_MONOVOL(MIXTYPE), 7>(out, frameCount, in, aux, vol, volinc, vola, volainc);
            break;
        case 8:
            volumeRampMulti<MIXTYPE_MONOVOL(MIXTYPE), 8>(out, frameCount, in, aux, vol, volinc, vola, volainc);
            break;
    }
}
...
void AudioMixer::volumeMix(TO* out, size_t outFrames, const TI* in, TA* aux, bool ramp, AudioMixer::track_t* t)
{
    if (USEFLOATVOL)
    {
        if (ramp)
        {
            volumeRampMulti<MIXTYPE>(t->mMixerChannelCount, out, outFrames, in, aux, t->mPrevVolume, t->mVolumeInc,
        ...
        {
            volumeMulti<MIXTYPE>(t->mMixerChannelCount, out, outFrames, in, aux, t->mVolume, t->auxLevel);
        }
    }
    else
    {
        if (ramp)
        {
            volumeRampMulti<MIXTYPE>(t->mMixerChannelCount, out, outFrames, in, aux, t->prevVolume, t->volumeInc,
                                     &t->prevAuxLevel, t->auxInc);
            if (ADJUSTVOL)
            {
                t->adjustVolumeRamp(aux != NULL);
        ...
        {
            volumeMulti<MIXTYPE>(t->mMixerChannelCount, out, outFrames, in, aux, t->volume, t->auxLevel);
        }
    }
}
    ...
    for (size_t numFrames = state->frameCount; numFrames;)
    {
        ...
        if (in == NULL || (((uintptr_t)in) & 3))
        {
            ...
            ALOGE_IF((((uintptr_t)in) & 3), "process_NoResampleOneTrack: bus error: "
                                            "buffer %p track %p, channels %d, needs %#x", in, t, t->channelCount,
                     t->needs);
            return;
        }

        const size_t outFrames = b.frameCount;
        volumeMix<MIXTYPE, is_same<TI, float>::value, false>(out, outFrames, in, aux, ramp, t);
...
void AudioMixer::track__Resample(track_t* t, TO* out, size_t outFrameCount, TO* temp, TA* aux)
{
    ...
    if (ramp || aux != NULL)
    {
        ...
        volumeMix<MIXTYPE, is_same<TI, float>::value, true>(out, outFrameCount, temp, aux, ramp, t);

    }
    else
    { // constant volume gain
        t->resampler->setVolume(t->mVolume[0], t->mVolume[1]);
 ...
 */
template <int MIXTYPE, typename TO, typename TI, typename TA>
void AudioMixer::track__NoResample(track_t* t, TO* out, size_t frameCount, TO* temp __unused, TA* aux)
{
    ALOGVV("track__NoResample\n");
    const TI* in = static_cast<const TI*>(t->in);

    volumeMix<MIXTYPE, is_same<TI, float>::value, true>(out, frameCount, in, aux, t->needsRamp(), t);
 ...
 */
void AudioMixer::convertMixerFormat(void* out, audio_format_t mixerOutFormat, void* in, audio_format_t mixerInFormat,
                                    size_t sampleCount)
{
 ...
 */
AudioMixer::hook_t AudioMixer::getTrackHook(int trackType, uint32_t channelCount, audio_format_t mixerInFormat,
                                            audio_format_t mixerOutFormat __unused)
{
    ...
    switch (trackType)
    {
        ...
        case TRACKTYPE_RESAMPLE:
            ...
            {
                case AUDIO_FORMAT_PCM_FLOAT:
                    return (AudioMixer::hook_t)track__Resample<MIXTYPE_MULTI, float /*TO*/, float /*TI*/, int32_t /*TA*/>;
                case AUDIO_FORMAT_PCM_16_BIT:
        ...
        case TRACKTYPE_NORESAMPLEMONO:
            ...
            {
                case AUDIO_FORMAT_PCM_FLOAT:
                    return (AudioMixer::hook_t)track__NoResample<MIXTYPE_MONOEXPAND, float, float, int32_t>;
                case AUDIO_FORMAT_PCM_16_BIT:
                    return (AudioMixer::hook_t)track__NoResample<MIXTYPE_MONOEXPAND, int32_t, int16_t, int32_t>;
                default:
                    LOG_ALWAYS_FATAL("bad mixerInFormat: %#x", mixerInFormat);
                    break;
            }
        ...
        case TRACKTYPE_NORESAMPLE:
            ...
            {
                case AUDIO_FORMAT_PCM_FLOAT:
                    return (AudioMixer::hook_t)track__NoResample<MIXTYPE_MULTI, float, float, int32_t>;
                case AUDIO_FORMAT_PCM_16_BIT:
                    return (AudioMixer::hook_t)track__NoResample<MIXTYPE_MULTI, int32_t, int16_t, int32_t>;
                default:
                    LOG_ALWAYS_FATAL("bad mixerInFormat: %#x", mixerInFormat);
                    break;
            }
...
{
    if (processType != PROCESSTYPE_NORESAMPLEONETRACK)
    { // Only NORESAMPLEONETRACK
    ...
    switch (mixerInFormat)
    {
        case AUDIO_FORMAT_PCM_FLOAT:
            ...
            {
                case AUDIO_FORMAT_PCM_FLOAT:
                    return process_NoResampleOneTrack<MIXTYPE_MULTI_SAVEONLY, float /*TO*/, float /*TI*/, int32_t /*TA*/>;
                case AUDIO_FORMAT_PCM_16_BIT:
                    return process_NoResampleOneTrack<MIXTYPE_MULTI_SAVEONLY, int16_t, float, int32_t>;
                default:
                    LOG_ALWAYS_FATAL("bad mixerOutFormat: %#x", mixerOutFormat);
                    break;
            }
        ...
        case AUDIO_FORMAT_PCM_16_BIT:
            ...
            {
                case AUDIO_FORMAT_PCM_FLOAT:
                    return process_NoResampleOneTrack<MIXTYPE_MULTI_SAVEONLY, float, int16_t, int32_t>;
                case AUDIO_FORMAT_PCM_16_BIT:
                    return process_NoResampleOneTrack<MIXTYPE_MULTI_SAVEONLY, int16_t, int16_t, int32_t>;
                default:
                    LOG_ALWAYS_FATAL("bad mixerOutFormat: %#x", mixerOutFormat);
                    break;
            }
