...
Q30_fmt(1.0), Q30_fmt(1.18920711500272), Q30_fmt(1.41421356237310), Q30_fmt(1.68179283050743)
};

const int32 two_cubic_roots[7] = {
Q29_fmt(0), Q29_fmt(1.25992104989487), Q29_fmt(1.58740105196820), Q29_fmt(2.00000000000000), Q29_fmt(2.51984209978975)
, Q29_fmt(3.17480210393640), Q29_fmt(3.99999999999999)
};
...
void pvmp3_dequantize_sample(int32 is[SUBBANDS_NUMBER * FILTERBANK_BANDS], mp3ScaleFactors* scalefac,
                             granuleInfo* gr_info, int32 used_freq_lines, mp3Header* info)
{
    int32 ss;
    int32 cb = 0;
    int32 global_gain;
            ...
            if (ss == next_cb_boundary)
            {
                ...
                if (gr_info->mixed_block_flag)
                {
                    ...
                    {   /*  1st 2 subbands of switched blocks */
                        global_gain = (gr_info->global_gain);
                        global_gain -= (1 + gr_info->scalefac_scale) * (scalefac->l[cb] + gr_info->preflag * pretab[cb]) << 1;
        ...
        for (cb = 0; cb < 22; cb++)
        {
            ...
            if (used_freq_lines >= mp3_sfBandIndex[sfreq].l[cb + 1])
            {
                if (global_gain <= 0)
                {
                    ...
                    {
                        pv_memset(&is[mp3_sfBandIndex[sfreq].l[cb]], 0,
    ...
    pv_memset(&is[used_freq_lines], 0, (FILTERBANK_BANDS * SUBBANDS_NUMBER - used_freq_lines) * sizeof(*is));
