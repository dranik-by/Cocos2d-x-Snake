...
int32 pvmp3_huffman_parsing(int32 is[SUBBANDS_NUMBER * FILTERBANK_BANDS], granuleInfo* grInfo, tmp3dec_file* pVars,
                            int32 part2_start, mp3Header* info)
{
    int32 i;
    int32 region1Start;
    int32 region2Start;
    ...
    while ((pMainData->usedBits < grBits) && (i < FILTERBANK_BANDS * SUBBANDS_NUMBER - 4))
    {
        pvmp3_huffman_quad_decoding(h, &is[i], pMainData);
        i += 4;
    }

    if ((pMainData->usedBits < grBits) && (i < FILTERBANK_BANDS * SUBBANDS_NUMBER))
    {
        pvmp3_huffman_quad_decoding(h, &is[i], pMainData);
        i += 4;
