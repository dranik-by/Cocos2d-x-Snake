...
ERROR_CODE pvmp3_framedecoder(tPVMP3DecoderExternal* pExt, void* pMem)
{
    ...
    errorCode = pvmp3_decode_header(&pVars->inputStream, info, &computed_crc);
    ...
    int32 outputFrameSize = (info->version_x == MPEG_1) ? 2 * SUBBANDS_NUMBER * FILTERBANK_BANDS : SUBBANDS_NUMBER * FILTERBANK_BANDS;
    ...
    if (info->layer_description == 3)
    {
        ...
        errorCode = pvmp3_get_side_info(&pVars->inputStream, &pVars->sideInfo, info, &computed_crc);
        ...
        if (bytes_to_discard < 0 || crc_error_count)
        {
            ...
            pv_memset((void*)pChVars[RIGHT]->work_buf_int32, 0,
                      SUBBANDS_NUMBER * FILTERBANK_BANDS * sizeof(pChVars[RIGHT]->work_buf_int32[0]));

            pv_memset((void*)pChVars[LEFT]->work_buf_int32, 0,
                      SUBBANDS_NUMBER * FILTERBANK_BANDS * sizeof(pChVars[LEFT]->work_buf_int32[0]));

            /*  clear circular buffers, to avoid any glitch */
            pv_memset((void*)&pChVars[LEFT]->circ_buffer[576], 0, 480 * sizeof(pChVars[LEFT]->circ_buffer[0]));
            pv_memset((void*)&pChVars[RIGHT]->circ_buffer[576], 0, 480 * sizeof(pChVars[RIGHT]->circ_buffer[0]));
        ...
        for (gr = 0; gr < (1 + !(info->version_x)); gr++)
        {
            if (errorCode != NO_ENOUGH_MAIN_DATA_ERROR)
            {
                for (ch = 0; ch < pVars->num_channels; ch++)
                {
                    ...
                    if (info->version_x == MPEG_1)
                    {
                        ...
                        pvmp3_get_scale_factors(&pVars->scaleFactors[ch], &pVars->sideInfo, gr, ch,
                                                &pVars->mainDataStream);
                    }
                    else
                    {
                        int32* tmp = pVars->Scratch_mem;
                        pvmp3_mpeg2_get_scale_factors(&pVars->scaleFactors[ch], &pVars->sideInfo, gr, ch, info,
                                                      (uint32*)tmp, &pVars->mainDataStream);
                    }

                    pChVars[ch]->used_freq_lines = pvmp3_huffman_parsing(pChVars[ch]->work_buf_int32,
                                                                         &pVars->sideInfo.ch[ch].gran[gr], pVars,
                                                                         part2_start, info);

                    pvmp3_dequantize_sample(pChVars[ch]->work_buf_int32, &pVars->scaleFactors[ch],
                                            &pVars->sideInfo.ch[ch].gran[gr], pChVars[ch]->used_freq_lines, info);
                ...
                if (pVars->num_channels == 2)
                {
                    ...
                    int32 used_freq_lines = (pChVars[LEFT]->used_freq_lines > pChVars[RIGHT]->used_freq_lines) ? pChVars[LEFT]->used_freq_lines : pChVars[RIGHT]->used_freq_lines;
                    ...
                    if (info->version_x == MPEG_1)
                    {
                        pvmp3_stereo_proc(pChVars[LEFT]->work_buf_int32, pChVars[RIGHT]->work_buf_int32,
                                          &pVars->scaleFactors[RIGHT], &pVars->sideInfo.ch[LEFT].gran[gr],
                                          used_freq_lines, info);
                    }
                    else
                    {
                        int32* tmp = pVars->Scratch_mem;
                        pvmp3_mpeg2_stereo_proc(pChVars[LEFT]->work_buf_int32, pChVars[RIGHT]->work_buf_int32,
                                                &pVars->scaleFactors[RIGHT], &pVars->sideInfo.ch[LEFT].gran[gr],
                                                &pVars->sideInfo.ch[RIGHT].gran[gr], (uint32*)tmp, used_freq_lines,
                                                info);
                    }
                }
            ...
            for (ch = 0; ch < pVars->num_channels; ch++)
            {
                ...
                pvmp3_reorder(pChVars[ch]->work_buf_int32, &pVars->sideInfo.ch[ch].gran[gr],
                              &pChVars[ch]->used_freq_lines, info, pVars->Scratch_mem);

                pvmp3_alias_reduction(pChVars[ch]->work_buf_int32, &pVars->sideInfo.ch[ch].gran[gr],
                                      &pChVars[ch]->used_freq_lines, info);
                ...
                if (pVars->sideInfo.ch[ch].gran[gr].mixed_block_flag && pVars->sideInfo.ch[ch].gran[gr].window_switching_flag)
                {
                ...
                pvmp3_imdct_synth(pChVars[ch]->work_buf_int32, pChVars[ch]->overlap,
                                  pVars->sideInfo.ch[ch].gran[gr].block_type, mixedBlocksLongBlocks,
                                  pChVars[ch]->used_freq_lines, pVars->Scratch_mem);
                ...
                pvmp3_poly_phase_synthesis(pChVars[ch], pVars->num_channels, pExt->equalizerType, &ptrOutBuffer[ch]);
...
__inline void fillDataBuf(tmp3Bits* pMainData, uint32 val)       /* val to write into the buffer */
{
...
void pvmp3_InitDecoder(tPVMP3DecoderExternal* pExt, void* pMem)
{
    ...
    pv_memset((void*)pVars->mainDataBuffer, 0, BUFSIZE * sizeof(*pVars->mainDataBuffer));
...
void pvmp3_resetDecoder(void* pMem)
{
    ...
    pv_memset((void*)pChVars[LEFT]->overlap, 0, SUBBANDS_NUMBER * FILTERBANK_BANDS * sizeof(pChVars[LEFT]->overlap[0]));

    pv_memset((void*)pChVars[RIGHT]->overlap, 0,
    ...
    pv_memset((void*)&pVars->scaleFactors[RIGHT], 0, sizeof(mp3ScaleFactors));

    pv_memset((void*)&pVars->scaleFactors[LEFT], 0, sizeof(mp3ScaleFactors));

    pv_memset((void*)&pVars->sideInfo, 0, sizeof(mp3SideInfo));
