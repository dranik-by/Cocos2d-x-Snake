...
ERROR_CODE pvmp3_get_side_info(tmp3Bits* inputStream, mp3SideInfo* si, mp3Header* info, uint32* crc)
{
    int32 ch, gr;
    uint32 tmp;
        ...
        for (gr = 0; gr < 2; gr++)
        {
            for (ch = 0; ch < stereo; ch++)
            {
                ...
                if (si->ch[ch].gran[gr].window_switching_flag)
                {
                    ...
                    else if ((si->ch[ch].gran[gr].block_type == 2) && (si->ch[ch].gran[gr].mixed_block_flag == 0))
                    {
    ...
    else /* Layer 3 LSF */
    {
        ...
        for (ch = 0; ch < stereo; ch++)
        {
            ...
            if (si->ch[ch].gran[0].window_switching_flag)
            {
                ...
                else if ((si->ch[ch].gran[0].block_type == 2) && (si->ch[ch].gran[0].mixed_block_flag == 0))
                {
