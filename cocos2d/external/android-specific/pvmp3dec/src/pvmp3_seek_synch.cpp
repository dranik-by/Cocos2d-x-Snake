...
ERROR_CODE pvmp3_frame_synch(tPVMP3DecoderExternal* pExt, void* pMem) /* bit stream structure */
{
    uint16 val;
    ERROR_CODE err;
    ...
    if (err == NO_DECODING_ERROR)
    {
        ...
        {
            int32 numBytes = fxp_mul32_Q28(mp3_bitrate[version][(temp << 16) >> 28] << 20, inv_sfreq[freq_index]);
