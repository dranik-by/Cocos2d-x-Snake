...
std::string GenerateFBS(const Parser &parser, const std::string &file_name, const GeneratorOptions &opts)
{
    ...
    {
        if (it != name_space->components.begin())
            schema += ".";
        schema += *it;
    }
    schema += ";\n\n";
    ...
    for (auto it = parser.structs_.vec.begin(); it != parser.structs_.vec.end(); ++it)
    {
        ...
        for (auto it2 = struct_def.fields.vec.begin(); it2 != struct_def.fields.vec.end(); ++it2)
        {
            auto &field = **it2;
...
{
    return SaveFile((path + file_name + ".fbs").c_str(), GenerateFBS(parser, file_name, opts), false);
}
