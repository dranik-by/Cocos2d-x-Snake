...
static std::string GenTypeBasic(const Parser &parser, const Type &type, bool real_enum)
{
    ...
    return real_enum && type.enum_def ? WrapInNameSpace(parser, type.enum_def->defined_namespace,
                                                        type.enum_def->name) : ctypename[type.base_type];
}
...
static std::string GenTypePointer(const Parser &parser, const Type &type)
{
    switch (type.base_type)
    {
        ...
        {
            return WrapInNameSpace(parser, type.struct_def->defined_namespace, type.struct_def->name);
        }
...
static std::string GenTypeWire(const Parser &parser, const Type &type, const char* postfix, bool real_enum)
{
    return IsScalar(type.base_type) ? GenTypeBasic(parser, type, real_enum) + postfix : IsStruct(
    type) ? "const " + GenTypePointer(parser, type) + " *" : "flatbuffers::Offset<" + GenTypePointer(parser,
                                                                                                     type) + ">" + postfix;
}
...
{
    return IsScalar(type.base_type) ? GenTypeBasic(parser, type, false) : IsStruct(type) ? GenTypePointer(parser,
                                                                                                          type) : "flatbuffers::uoffset_t";
}

// Return a C++ type for any type (scalar/pointer) specifically for
// using a flatbuffer.
static std::string GenTypeGet(const Parser &parser, const Type &type, const char* afterbasic, const char* beforeptr,
                              const char* afterptr, bool real_enum)
{
    return IsScalar(type.base_type) ? GenTypeBasic(parser, type, real_enum) + afterbasic : beforeptr + GenTypePointer(
    parser, type) + afterptr;
}

static std::string GenEnumVal(const EnumDef &enum_def, const EnumVal &enum_val, const GeneratorOptions &opts)
{
    return opts.prefixed_enums ? enum_def.name + "_" + enum_val.name : enum_val.name;
}
...
{
    return field.value.type.enum_def && IsScalar(field.value.type.base_type) ? "static_cast<" + GenTypeBasic(parser,
                                                                                                             field.value.type,
                                                                                                             from) + ">(" + val + ")" : val;
}
...
static void GenTable(const Parser &parser, StructDef &struct_def, const GeneratorOptions &opts, std::string* code_ptr)
{
    ...
    for (auto it = struct_def.fields.vec.begin(); it != struct_def.fields.vec.end(); ++it)
    {
        ...
        {  // Deprecated fields won't be accessible.
            ...
            std::string call = IsScalar(field.value.type.base_type) ? "GetField<" : (IsStruct(
            field.value.type) ? "GetStruct<" : "GetPointer<");
    ...
    for (auto it = struct_def.fields.vec.begin(); it != struct_def.fields.vec.end(); ++it)
    {
        ...
        if (!field.deprecated)
        {
            ...
            if (field.value.type.enum_def && IsScalar(field.value.type.base_type))
            {
                ...
                if (ev)
                {
                    code += WrapInNameSpace(parser, field.value.type.enum_def->defined_namespace,
                                            GenEnumVal(*field.value.type.enum_def, *ev, opts));
                }
                else
                {
...
static void GenStruct(const Parser &parser, StructDef &struct_def, std::string* code_ptr)
{
    ...
    for (auto it = struct_def.fields.vec.begin(); it != struct_def.fields.vec.end(); ++it)
    {
        ...
        code += "  " + GenTypeGet(parser, field.value.type, " ", "const ", " &", true);
        code += field.name + "() const { return ";
        code += GenUnderlyingCast(parser, field, true, IsScalar(
        field.value.type.base_type) ? "flatbuffers::EndianScalar(" + field.name + "_)" : field.name + "_");
        code += "; }\n";
    }
...
std::string GenerateCPP(const Parser &parser, const std::string &file_name, const GeneratorOptions &opts)
{
    ...
    {
        if (!(**it).fixed)
            GenTable(parser, **it, opts, &decl_code);
    }
    ...
    if (enum_code.length() || decl_code.length())
    {
        ...
        include_guard_ident.erase(std::remove_if(include_guard_ident.begin(), include_guard_ident.end(), [](char c)
        {
            return !isalnum(c);
