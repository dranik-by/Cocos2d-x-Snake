...
int64_t Parser::ParseHexNum(int nibbles)
{
    for (int i = 0; i < nibbles; i++)
        if (!isxdigit(cursor_[i]))
            Error("escape code must be followed by " + NumToString(nibbles) + " hex digits");
        ...
        switch (c)
        {
            ...
            if (attribute_ == IDLTYPE) { \
              ...
              return; \
                ...
                {
                    const char* start = cursor_ - 1;
                    while (isdigit(static_cast<unsigned char>(*cursor_)))
                        cursor_++;
                    if (*cursor_ == '.')
                    {
                        cursor_++;
...
FieldDef &Parser::AddField(StructDef &struct_def, const std::string &name, const Type &type)
{
    ...
    if (struct_def.fixed)
    {  // statically compute the field offset
...
void Parser::ParseField(StructDef &struct_def)
{
    ...
    if (type.enum_def && IsScalar(type.base_type) && !struct_def.fixed && !type.enum_def->attributes.Lookup(
    "bit_flags") && !type.enum_def->ReverseLookup(static_cast<int>(
                                                  StringToInt(field.value.constant.c_str()))))
        Error(
        "enum " + type.enum_def->name + " does not have a declaration for this field\'s default of " + field.value.constant);
    ...
    if (field.required && (struct_def.fixed || IsScalar(field.value.type.base_type)))
    ...
    if (nested)
    {
        ...
        if (field.value.type.base_type != BASE_TYPE_VECTOR || field.value.type.element != BASE_TYPE_UCHAR)
...
void Parser::ParseAnyValue(Value &val, FieldDef* field)
{
    switch (val.type.base_type)
    {
        ...
        {
            assert(field);
            if (!field_stack_.size() || field_stack_.back().second->value.type.base_type != BASE_TYPE_UTYPE)
            ...
            if (!enum_val)
                Error("illegal type id for: " + field->name);
...
uoffset_t Parser::ParseTable(const StructDef &struct_def)
{
    ...
    if (struct_def.fixed)
    {
        ...
        struct_stack_.insert(struct_stack_.end(), builder_.GetBufferPointer(),
...
bool Parser::TryTypedValue(int dtoken, bool check, Value &e, BaseType req)
{
    ...
    if (match)
    {
        ...
        if (!check)
        {
            ...
            {
                Error(std::string(
                "type mismatch: expecting: ") + kTypeNames[e.type.base_type] + ", found: " + kTypeNames[req]);
            }
        }
        Next();
    }
...
int64_t Parser::ParseIntegerFromString(Type &type)
{
    ...
    do
    {
        ...
        {  // The field has an enum type
            ...
            if (!enum_val)
                Error("unknown enum value: " + word + ", for enum: " + type.enum_def->name);
        ...
        {  // No enum type, probably integral field.
            ...
            if (!dot)
                Error("enum values need to be qualified by an enum type");
            ...
            if (!enum_def)
                Error("unknown enum: " + enum_def_str);
            auto enum_val = enum_def->vals.Lookup(enum_val_str);
            if (!enum_val)
                Error("unknown enum value: " + enum_val_str);
            result |= enum_val->value;
        }
    } while (*next);
    return result;
...
void Parser::ParseSingleValue(Value &e)
{
    ...
    else if (TryTypedValue(kTokenIntegerConstant, IsScalar(e.type.base_type), e, BASE_TYPE_INT) || TryTypedValue(
    kTokenFloatConstant, IsFloat(e.type.base_type), e, BASE_TYPE_FLOAT) || TryTypedValue(kTokenStringConstant,
...
void Parser::ParseEnum(bool is_union)
{
    ...
    if (enums_.Add(name, &enum_def))
        Error("enum already exists: " + name);
    if (is_union)
    {
    ...
    Expect('{');
    if (is_union)
        enum_def.vals.Add("NONE", new EnumVal("NONE", 0));
    do
    {
        name = attribute_;
        dc = doc_comment_;
    ...
    if (enum_def.attributes.Lookup("bit_flags"))
    {
        ...
        {
            if (static_cast<size_t>((*it)->value) >= SizeOf(enum_def.underlying_type.base_type) * 8)
...
StructDef &Parser::StartStruct()
{
    ...
    if (!struct_def.predecl)
        Error("datatype already exists: " + name);
...
void Parser::ParseDecl()
{
        ...
        if (num_id_fields)
        {
            ...
            for (int i = 0; i < static_cast<int>(fields.size()); i++)
            {
                if (i != atoi(fields[i]->attributes.Lookup("id")->constant.c_str()))
                    Error("field id\'s must be consecutive from 0, id " + NumToString(i) + " missing or set twice");
    ...
    auto CheckClash = [&fields, &struct_def](const char* suffix, BaseType basetype)
    {
        ...
        {
            auto &name = (*it)->name;
            if (name.length() > len && name.compare(name.length() - len, len,
                                                    suffix) == 0 && (*it)->value.type.base_type != BASE_TYPE_UTYPE)
            {
                auto field = struct_def.fields.Lookup(name.substr(0, name.length() - len));
                if (field && field->value.type.base_type == basetype)
                    Error("Field " + name + " would clash with generated functions for field " + field->name);
            }
        }
    };
...
bool Parser::Parse(const char* source, const char** include_paths, const char* source_filename)
{
    ...
    try
    {
        ...
        while (token_ != kTokenEof)
        {
            ...
            {
                if (!root_struct_def)
                    Error("no root type set to parse json with");
                if (builder_.GetSize())
                {
            ...
            else if (token_ == kTokenFileIdentifier)
            {
                ...
                if (file_identifier_.length() != FlatBufferBuilder::kFileIdentifierLength)
                    Error("file_identifier must be exactly " + NumToString(
                    FlatBufferBuilder::kFileIdentifierLength) + " characters");
                Expect(';');
            }
