    ...
    * @param q The other quaternion */
    ...
    btQuaternion slerp(const btQuaternion &q, const btScalar &t) const
    {
        ...
        if (btFabs(product) < btScalar(1))
        {
            ...
            return btQuaternion((m_floats[0] * s0 + q.x() * s1) * d, (m_floats[1] * s0 + q.y() * s1) * d,
...
SIMD_FORCE_INLINE btQuaternion operator*(const btQuaternion &q, const btVector3 &w)
{
    ...
    #else
    return btQuaternion(q.w() * w.x() + q.y() * w.z() - q.z() * w.y(), q.w() * w.y() + q.z() * w.x() - q.x() * w.z(),
...
SIMD_FORCE_INLINE btQuaternion operator*(const btVector3 &w, const btQuaternion &q)
{
    ...
    #else
    return btQuaternion(+w.x() * q.w() + w.y() * q.z() - w.z() * q.y(), +w.y() * q.w() + w.z() * q.x() - w.x() * q.z(),
...
SIMD_FORCE_INLINE btQuaternion shortestArcQuat(const btVector3 &v0,
                                               const btVector3 &v1) // Game Programming Gems 2.10. make sure v0,v1 are normalized
{
