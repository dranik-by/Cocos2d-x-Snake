...
public:
    ...
    */
    ...
    btMatrix3x3(const btScalar &xx, const btScalar &xy, const btScalar &xz, const btScalar &yx, const btScalar &yy,
                const btScalar &yz, const btScalar &zx, const btScalar &zy, const btScalar &zz)
    {
    ...
    *  @param zz Bottom Right*/
    void setValue(const btScalar &xx, const btScalar &xy, const btScalar &xz, const btScalar &yx, const btScalar &yy,
                  const btScalar &yz, const btScalar &zx, const btScalar &zy, const btScalar &zz)
    {
    ...
    *  @param q The Quaternion to match */
    void setRotation(const btQuaternion &q)
    {
        ...
        setValue(btScalar(1.0) - (yy + zz), xy - wz, xz + wy, xy + wz, btScalar(1.0) - (xx + zz), yz - wx, xz - wy,
                 yz + wx, btScalar(1.0) - (xx + yy));
        #endif
    }
    ...
    **/
    ...
    void setIdentity()
    {
        ...
        m_el[2] = v0010;
        #else
        setValue(btScalar(1.0), btScalar(0.0), btScalar(0.0), btScalar(0.0), btScalar(1.0), btScalar(0.0),
    ...
    static const btMatrix3x3 &getIdentity()
    {
        ...
        #else
        static const btMatrix3x3 identityMatrix(btScalar(1.0), btScalar(0.0), btScalar(0.0), btScalar(0.0),
    ...
    * @param s Scaling vector The elements of the vector will scale each column */
    ...
    btMatrix3x3 scaled(const btVector3 &s) const
    {
        ...
        #else
        return btMatrix3x3(m_el[0].x() * s.x(), m_el[0].y() * s.y(), m_el[0].z() * s.z(), m_el[1].x() * s.x(),
...
SIMD_FORCE_INLINE btMatrix3x3 &btMatrix3x3::operator+=(const btMatrix3x3 &m)
{
    ...
    #else
    setValue(m_el[0][0] + m.m_el[0][0], m_el[0][1] + m.m_el[0][1], m_el[0][2] + m.m_el[0][2], m_el[1][0] + m.m_el[1][0],
             m_el[1][1] + m.m_el[1][1], m_el[1][2] + m.m_el[1][2], m_el[2][0] + m.m_el[2][0], m_el[2][1] + m.m_el[2][1],
             m_el[2][2] + m.m_el[2][2]);
    #endif
    return *this;
}
...
SIMD_FORCE_INLINE btMatrix3x3 operator*(const btMatrix3x3 &m, const btScalar &k)
{
    ...
    #else
    return btMatrix3x3(m[0].x() * k, m[0].y() * k, m[0].z() * k, m[1].x() * k, m[1].y() * k, m[1].z() * k, m[2].x() * k,
                       m[2].y() * k, m[2].z() * k);
    #endif
}
...
SIMD_FORCE_INLINE btMatrix3x3 operator+(const btMatrix3x3 &m1, const btMatrix3x3 &m2)
{
    ...
    #else
    return btMatrix3x3(m1[0][0] + m2[0][0], m1[0][1] + m2[0][1], m1[0][2] + m2[0][2],
...
SIMD_FORCE_INLINE btMatrix3x3 btMatrix3x3::absolute() const
{
    ...
    #else
    return btMatrix3x3(btFabs(m_el[0].x()), btFabs(m_el[0].y()), btFabs(m_el[0].z()), btFabs(m_el[1].x()),
                       btFabs(m_el[1].y()), btFabs(m_el[1].z()), btFabs(m_el[2].x()), btFabs(m_el[2].y()),
                       btFabs(m_el[2].z()));
    #endif
}
...
SIMD_FORCE_INLINE btMatrix3x3 btMatrix3x3::transpose() const
{
    ...
    return btMatrix3x3( v0, v1, v2 );
    #else
    return btMatrix3x3(m_el[0].x(), m_el[1].x(), m_el[2].x(), m_el[0].y(), m_el[1].y(), m_el[2].y(), m_el[0].z(),
                       m_el[1].z(), m_el[2].z());
    #endif
}
...
{
    return btMatrix3x3(cofac(1, 1, 2, 2), cofac(0, 2, 2, 1), cofac(0, 1, 1, 2), cofac(1, 2, 2, 0), cofac(0, 0, 2, 2),
...
SIMD_FORCE_INLINE btMatrix3x3 btMatrix3x3::inverse() const
{
    ...
    return btMatrix3x3(co.x() * s, cofac(0, 2, 2, 1) * s, cofac(0, 1, 1, 2) * s, co.y() * s, cofac(0, 0, 2, 2) * s,
...
//    static const __m128i xyzMask = (const __m128i){ -1ULL, 0xffffffffULL };
    ...
    return btMatrix3x3( r0, r1, r2 );
    #else
    return btMatrix3x3(m_el[0].x() * m[0].x() + m_el[1].x() * m[1].x() + m_el[2].x() * m[2].x(),
...
SIMD_FORCE_INLINE btMatrix3x3 btMatrix3x3::timesTranspose(const btMatrix3x3 &m) const
{
    ...
    #else
    return btMatrix3x3(m_el[0].dot(m[0]), m_el[0].dot(m[1]), m_el[0].dot(m[2]), m_el[1].dot(m[0]), m_el[1].dot(m[1]),
...
SIMD_FORCE_INLINE btMatrix3x3 operator*(const btMatrix3x3 &m1, const btMatrix3x3 &m2)
{
    ...
    #else
    return btMatrix3x3(m2.tdotx(m1[0]), m2.tdoty(m1[0]), m2.tdotz(m1[0]), m2.tdotx(m1[1]), m2.tdoty(m1[1]),
