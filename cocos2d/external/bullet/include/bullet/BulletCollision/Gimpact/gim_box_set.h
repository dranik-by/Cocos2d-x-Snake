...
protected:
    GUINT _sort_and_calc_splitting_index(gim_array<GIM_AABB_DATA> &primitive_boxes, GUINT startIndex, GUINT endIndex,
                                         GUINT splitAxis);
...
public:
    ...
    //! returns the indices of the primitives in the m_primitive_manager
    SIMD_FORCE_INLINE bool boxQueryTrans(const GIM_AABB &box, const btTransform &transform,
    ...
    //! returns the indices of the primitives in the m_primitive_manager
    SIMD_FORCE_INLINE bool rayQuery(const btVector3 &ray_dir, const btVector3 &ray_origin,
                                    gim_array<GUINT> &collided_results) const
    {
        GUINT curIndex = 0;
        GUINT numNodes = getNodeCount();
...
protected:
    ...
    SIMD_FORCE_INLINE bool node_collision(GUINT node0, GUINT node1)
    {
        ...
        if (t0_is_trimesh && node0_is_leaf)
        {
            ...
            result = m_box1.collide_triangle_exact(m_tri0.m_vertices[0], m_tri0.m_vertices[1], m_tri0.m_vertices[2],
                                                   m_tri0_plane);
        ...
        else if (t1_is_trimesh && node1_is_leaf)
        {
            ...
            result = m_box0.collide_triangle_exact(m_tri1.m_vertices[0], m_tri1.m_vertices[1], m_tri1.m_vertices[2],
                                                   m_tri1_plane);
...
public:
    void find_collision(BOX_SET_CLASS0* boxset1, const btTransform &trans1, BOX_SET_CLASS1* boxset2,
                        const btTransform &trans2, gim_pair_set &collision_pairs, bool complete_primitive_tests = true)
    {
