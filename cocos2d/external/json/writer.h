...
protected:
    ...
    bool WriteString(const Ch* str, SizeType length)
    {
        ...
        while (ScanWriteUnescapedString(is, length))
        {
            ...
            else if ((sizeof(Ch) == 1 || static_cast<unsigned>(c) < 256) && RAPIDJSON_UNLIKELY(
            ...
            !(writeFlags & kWriteValidateEncodingFlag ? Transcoder<SourceEncoding, TargetEncoding>::Validate(is,
                                                                                                             *os_) : Transcoder<SourceEncoding, TargetEncoding>::TranscodeUnsafe(
            is, *os_))))
                return false;
        }
        PutUnsafe(*os_, '\"');
