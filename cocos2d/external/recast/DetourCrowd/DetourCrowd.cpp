...
static int getNeighbours(const float* pos, const float height, const float range, const dtCrowdAgent* skip,
                         dtCrowdNeighbour* result, const int maxResult, dtCrowdAgent** agents, const int /*nagents*/,
    ...
    for (int i = 0; i < nagents; ++i)
    {
        ...
        if (ag->targetState != DT_CROWDAGENT_TARGET_NONE && ag->targetState != DT_CROWDAGENT_TARGET_FAILED)
        {
            if (!m_navquery->isValidPolyRef(ag->targetRef, &m_filters[ag->params.queryFilterType]))
            {
                ...
                ag->targetRef = 0;
                m_navquery->findNearestPoly(ag->targetPos, m_ext, &m_filters[ag->params.queryFilterType],
                                            &ag->targetRef, nearest);
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo* debug)
{
    ...
    for (int i = 0; i < nagents; ++i)
    {
        ...
        if (dtVdist2DSqr(ag->npos, ag->boundary.getCenter()) > dtSqr(updateThr) || !ag->boundary.isValid(m_navquery,
                                                                                                         &m_filters[ag->params.queryFilterType]))
        {
        ...
        ag->nneis = getNeighbours(ag->npos, ag->params.height, ag->params.collisionQueryRange, ag, ag->neis,
                                  DT_CROWDAGENT_MAX_NEIGHBOURS, agents, nagents, m_grid);
    ...
    for (int i = 0; i < nagents; ++i)
    {
        ...
        if (overOffmeshConnection(ag, triggerRadius))
        {
            ...
            dtPolyRef refs[2];
            if (ag->corridor.moveOverOffmeshConnection(ag->cornerPolys[ag->ncorners - 1], refs, anim->startPos,
    ...
    for (int i = 0; i < nagents; ++i)
    {
        ...
        if (ag->params.updateFlags & DT_CROWD_OBSTACLE_AVOIDANCE)
        {
            ...
            if (adaptive)
            {
                ns = m_obstacleQuery->sampleVelocityAdaptive(ag->npos, ag->params.radius, ag->desiredSpeed, ag->vel,
                                                             ag->dvel, ag->nvel, params, vod);
            }
            else
            {
                ns = m_obstacleQuery->sampleVelocityGrid(ag->npos, ag->params.radius, ag->desiredSpeed, ag->vel,
                                                         ag->dvel, ag->nvel, params, vod);
            }
