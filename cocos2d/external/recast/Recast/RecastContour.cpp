...
static void simplifyContour(rcIntArray &points, rcIntArray &simplified, const float maxError, const int maxEdgeLen,
...
static void mergeRegionHoles(rcContext* ctx, rcContourRegion &region)
{
    ...
    rcScopedDelete<rcPotentialDiagonal> diags = (rcPotentialDiagonal*)rcAlloc(sizeof(rcPotentialDiagonal) * maxVerts,
                                                                              RC_ALLOC_TEMP);
    if (!diags)
    {
...
bool rcBuildContours(rcContext* ctx, rcCompactHeightfield &chf, const float maxError, const int maxEdgeLen,
                     rcContourSet &cset, const int buildFlags)
{
    ...
    if (cset.nconts > 0)
    {
        ...
        if (nholes > 0)
        {
            ...
            rcScopedDelete<rcContourRegion> regions = (rcContourRegion*)rcAlloc(sizeof(rcContourRegion) * nregions,
                                                                                RC_ALLOC_TEMP);
            if (!regions)
            {
