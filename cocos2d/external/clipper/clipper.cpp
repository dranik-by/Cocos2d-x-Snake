...
public:
    ...
    Int128(const cInt &_hi, const ulong64 &_lo)
    : lo(_lo)
    , hi(_hi)
    {
    }
...
bool FindSegment(OutPt*&pp, bool UseFullInt64Range, IntPoint &pt1, IntPoint &pt2)
{
    ...
    do
    {
        if (SlopesEqual(pt1a, pt2a, pp->Pt, pp->Prev->Pt, UseFullInt64Range) && SlopesEqual(pt1a, pt2a, pp->Pt,
                                                                                            UseFullInt64Range) && GetOverlapSegment(
        pt1a, pt2a, pp->Pt, pp->Prev->Pt, pt1, pt2))
            return true;
        pp = pp->Next;
    } while (pp != pp2);
    return false;
    ...
    for (;;)
    {
        ...
        if (E->Prev == E->Next)
            break; //only two vertices
                                       ...
                                       m_UseFullRange) && (!m_PreserveCollinear || !Pt2IsBetweenPt1AndPt3(E->Prev->Curr,
                                                                                                          E->Curr,
                                                                                                          E->Next->Curr)))
        {
...
OutPt* Clipper::AddLocalMinPoly(TEdge* e1, TEdge* e2, const IntPoint &Pt)
{
    ...
    if (prevE && prevE->OutIdx >= 0 && (TopX(*prevE, Pt.Y) == TopX(*e, Pt.Y)) && SlopesEqual(*e, *prevE,
                                                                                             m_UseFullRange) && (e->WindDelta != 0) && (prevE->WindDelta != 0))
    {
...
void Clipper::InsertLocalMinimaIntoAEL(const cInt botY)
{
    while (m_CurrentLM && (m_CurrentLM->Y == botY))
    {
        ...
        *lb->PrevInAEL, *lb, m_UseFullRange) && (lb->WindDelta != 0) && (lb->PrevInAEL->WindDelta != 0))
        {
        ...
        if (lb->NextInAEL != rb)
        {
            ...
            if (rb->OutIdx >= 0 && rb->PrevInAEL->OutIdx >= 0 && SlopesEqual(*rb->PrevInAEL, *rb,
                                                                             m_UseFullRange) && (rb->WindDelta != 0) && (rb->PrevInAEL->WindDelta != 0))
            {
...
void Clipper::FixupOutPolygon(OutRec &outrec)
{
    ...
    for (;;)
    {
        ...
        if ((pp->Pt == pp->Next->Pt) || (pp->Pt == pp->Prev->Pt) || (SlopesEqual(pp->Prev->Pt, pp->Pt, pp->Next->Pt,
                                                                                 m_UseFullRange) && (!m_PreserveCollinear || !Pt2IsBetweenPt1AndPt3(
...
void MinkowskiSum(const Path &pattern, const Paths &paths, Paths &solution, PolyFillType pathFillType,
                  bool pathIsClosed)
{
    Clipper c;
...
void OffsetPaths(const Paths &in_polys, Paths &out_polys, double delta, JoinType jointype, EndType_ endtype,
                 double limit)
{
