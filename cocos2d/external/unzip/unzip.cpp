...
static int unzSearchCentralDir(const zlib_filefunc64_32_def* pzlib_filefunc_def, uint64_t* pos_found, voidpf filestream)
{
    ...
    *pos_found = 0;
    ...
    while (back_read < max_back)
    {
        ...
        read_size = ((BUFREADCOMMENT + 4) < (file_size - read_pos)) ? (BUFREADCOMMENT + 4) : (uint32_t)(file_size - read_pos);
        ...
        for (i = read_size - 3; (i--) > 0;)
            if (((*(buf + i)) == (ENDHEADERMAGIC & 0xff)) && ((*(buf + i + 1)) == (ENDHEADERMAGIC >> 8 & 0xff)) && ((*(buf + i + 2)) == (ENDHEADERMAGIC >> 16 & 0xff)) && ((*(buf + i + 3)) == (ENDHEADERMAGIC >> 24 & 0xff)))
            {
                                 ...
                                 const uint64_t endcentraloffset)
{
    ...
    *offset = 0;
    ...
    if (ZSEEK64(*pzlib_filefunc_def, filestream, endcentraloffset - SIZECENTRALHEADERLOCATOR,
...
static int unzGetCurrentFileInfoField(unzFile file, uint32_t* seek, void* field, uint16_t field_size,
                                      uint16_t size_file_field, int null_terminated_field)
{
    unz64_internal* s = NULL;
...
static int unzGetCurrentFileInfoInternal(unzFile file, unz_file_info64* pfile_info,
                                         unz_file_info64_internal* pfile_info_internal, char* filename,
                                         uint16_t filename_size, void* extrafield, uint16_t extrafield_size,
                                         char* comment, uint16_t comment_size)
{
    ...
    if (ZSEEK64(s->z_filefunc, s->filestream_with_CD, s->pos_in_central_dir + s->byte_before_the_zipfile,
                ZLIB_FILEFUNC_SEEK_SET) != 0)
        err = UNZ_ERRNO;
                                          ...
                                          char* comment, uint16_t comment_size)
{
    ...
    err = unzGetCurrentFileInfoInternal(file, &file_info64, NULL, filename, filename_size, extrafield, extrafield_size,
...
ZEXTERN int ZEXPORT unzGetCurrentFileInfo64(unzFile file, unz_file_info64* pfile_info, char* filename,
                                            ...
                                            char* comment, uint16_t comment_size)
{
    return unzGetCurrentFileInfoInternal(file, pfile_info, NULL, filename, filename_size, extrafield, extrafield_size,
                                         comment, comment_size);
}
...
static int unzCheckCurrentFileCoherencyHeader(unz64_internal* s, uint32_t* psize_variable,
                                              uint64_t* poffset_local_extrafield, uint16_t* psize_local_extrafield)
{
    uint32_t magic = 0;
    uint16_t value16 = 0;
    uint32_t value32 = 0;
...
ZEXTERN int ZEXPORT unzOpenCurrentFile3(unzFile file, int* method, int* level, int raw, const char* password)
{
    ...
    if (unzCheckCurrentFileCoherencyHeader(s, &size_variable, &offset_local_extrafield,
    ...
    if ((password != NULL) && ((s->cur_file_info.flag & 1) != 0))
    {
        ...
        #endif
        {
            ...
            expected = (s->cur_file_info.flag & (1 << 3)) ? s->cur_file_info.dos_date >> 8 : s->cur_file_info.crc >> 24;
...
ZEXTERN int ZEXPORT unzReadCurrentFile(unzFile file, voidp buf, uint32_t len)
{
    ...
    if ((s->pfile_in_zip_read->compression_method == 0) || (s->pfile_in_zip_read->raw))
    {
        if (len > s->pfile_in_zip_read->rest_read_compressed + s->pfile_in_zip_read->stream.avail_in)
            s->pfile_in_zip_read->stream.avail_out = (uInt)s->pfile_in_zip_read->rest_read_compressed + s->pfile_in_zip_read->stream.avail_in;
    }
    ...
    do
    {
        if (s->pfile_in_zip_read->stream.avail_in == 0)
        {
            ...
            if (s->pfile_in_zip_read->stream.next_in != NULL)
                bytes_not_read = (uint32_t)(s->pfile_in_zip_read->read_buffer + UNZ_BUFSIZE - s->pfile_in_zip_read->stream.next_in);
            ...
            if ((s->cur_file_info.flag & 1) != 0)
            {
                ...
                if (s->pcrc_32_tab != NULL)
                {
                    ...
                    for (i = 0; i < total_bytes_read; i++)
                        s->pfile_in_zip_read->read_buffer[i] = zdecode(s->keys, s->pcrc_32_tab,
        ...
        else
        {
            ...
            */
            ...
            s->pfile_in_zip_read->total_out_64 += out_bytes;
            s->pfile_in_zip_read->rest_read_uncompressed -= out_bytes;
            s->pfile_in_zip_read->crc32 = (uint32_t)crc32(s->pfile_in_zip_read->crc32, buf_before, (uint32_t)out_bytes);
...
ZEXTERN int ZEXPORT unzGoToFirstFile2(unzFile file, unz_file_info64* pfile_info, char* filename, uint16_t filename_size,
                                      void* extrafield, uint16_t extrafield_size, char* comment, uint16_t comment_size)
{
    ...
    err = unzGetCurrentFileInfoInternal(file, &s->cur_file_info, &s->cur_file_info_internal, filename, filename_size,
...
ZEXTERN int ZEXPORT unzGoToNextFile2(unzFile file, unz_file_info64* pfile_info, char* filename, uint16_t filename_size,
                                     void* extrafield, uint16_t extrafield_size, char* comment, uint16_t comment_size)
{
    unz64_internal* s = NULL;
    int err = UNZ_OK;
...
ZEXTERN int ZEXPORT unzSeek64(unzFile file, uint64_t offset, int origin)
{
    ...
    is_within_buffer = (s->pfile_in_zip_read->stream.avail_in != 0) && (s->pfile_in_zip_read->rest_read_compressed != 0 || s->cur_file_info.compressed_size < UNZ_BUFSIZE) && (position >= stream_pos_begin && position < stream_pos_end);
